-- Replace the removed passive social-post tracking schema with the intentional,
-- user-confirmed social-post reflection schema.
--
-- The legacy table used the same `social_posts` name, so migration 013's
-- CREATE TABLE IF NOT EXISTS could not install the new canonical-post schema.
-- The old records are deliberately discarded; this feature does not retain or
-- migrate passive tracking data.

BEGIN;

DROP TABLE IF EXISTS social_post_reflections CASCADE;
DROP TABLE IF EXISTS social_post_snapshots CASCADE;
...
CREATE INDEX idx_social_post_snapshots_post_observed
  ON social_post_snapshots (post_id, observed_at DESC);
CREATE INDEX idx_social_post_snapshots_observed
  ON social_post_snapshots (observed_at DESC);

CREATE TABLE social_post_reflections (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
